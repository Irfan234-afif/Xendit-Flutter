...
public class FxenditPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private void create(@NonNull MethodCall call, @NonNull final Result result) {
        if (call.hasArgument("publishedKey")) {
            init(call.argument("publishedKey"));
        }
        // xendit = new Xendit(activity.getApplication().getApplicationContext(),
        // "xnd_public_production_C4dllHAutlFUGUjdDJOnQs7UgFYFcYVRQHJIR5oMOaa7Np16dGQAXoMgEsefczW",
        // activity);
        // Card card = new Card("4000000000001091", "12", "2024", "123");

        Integer amount = call.argument("amount") != null ? call.argument("amount") : 0;
    ...
    private void check(@NonNull MethodCall call, @NonNull final Result result) {
        // if (call.hasArgument("publishedKey")) {
        init(call.<String>argument(
                "xnd_public_development_9aXTOZaH7x18wIOnQmS46duJe6n8I72vneT12v1APsVHPT4xpEdjmyT2HLwQHU"));
        // }

        Integer _amount = 25000;
        Boolean _shouldAuthenticate = true;
        ...
        Card card = new Card("4456530000001096",
                "12",
                "2024",
                "123");
